...
| seq_len | pad 到 **256** | 实验 3.H.3：最小满足约束的值，FFN 快 13% |
...
7. **Step 7** ✅：真实激活 Hook 实验（实验 3.H.1/2/3/5，2026-03-20）
   ...
   - 集成决策：hook `input_layernorm` output（attn）+ `post_attention_layernorm` output（ffn），pad seq=256，sf=2^16，每层两个独立文件
...
|------|------|-----|--------------|-------------|
| B1 图像替换 | 7/400 | 1.75% | [0.85%, 3.57%] | ✅ |
| B2 Embedding 替换 | 1/400 | 0.25% | [0.04%, 1.40%] | ✅ |
| 合计 | 8/800 | **1.00%** | [0.51%, 1.96%] | ✅ |
| 合法成员假阴性 | 0/50 | 0% | — | ✅ 零漏报 |
...
两者预期均为 1%，合计 8/800=1.00% 精确等于理论值。B1 7次、B2 1次的分布是随机波动：
- 两比例 z 检验 p≈0.033，仅处于临界，不足以判定结构差异
- 跨层绑定对两者均正常工作（SHA256 使两种输入均呈均匀随机，不存在结构偏差）
- 如重跑，很可能出现 3:5 或 5:3 等其他分布

**"多跑几次"的讨论**

多次重复实验与直接加大 N_TRIALS 统计效果完全等价（均为累积总试验次数）。800 次的 Wilson CI 已经足够窄，增加试验次数收益递减：
...
论文的说服力来自"理论推导 + 实验吻合"两层论证，而非堆砌试验次数。
